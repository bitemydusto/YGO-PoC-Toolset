	...
	for (size_t i = 0; i < 2; i++)
	{
		for (size_t j = 0; j < 5; j++)
		{
            ...
            if (intID != 0)
            {
                ...
                {
                    if (cardID == Cards::OBELISK_THE_TORMENTOR)
                    {
                        int index = FUN::IndexOfZoneEffect(i, j, Cards::OBELISK_THE_TORMENTOR);

						if (index >= 0)
						{
							FUN::RemoveEffectEntity(i, j, index);
						}
                    }
