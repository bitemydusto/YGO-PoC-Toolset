...
int __stdcall HasActivatableEffect(uint16_t cardID)
{
	return std::find(ActivatableCards.begin(), ActivatableCards.end(), cardID) != ActivatableCards.end();
}
int __stdcall GetSpellSpeed(uint16_t cardID)
{
	auto it = std::find_if(SpellSpeeds.begin(), SpellSpeeds.end(),
		[cardID](const SpellSpeed& speed) { return speed.CardID == cardID; });
	if (it != SpellSpeeds.end())
	{
		return it->Speed;
	}
	else
...
__declspec(naked) void PatchActivatableEffects()
{
	...
	hook:
		PUSH EAX
		PUSH EAX
		CALL OFFSET HasActivatableEffect
		TEST EAX, EAX
		POP EAX
		JZ hook_end
		MOV EAX, 0x0056813b
